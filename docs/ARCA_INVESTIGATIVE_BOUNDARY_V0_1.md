...
A source URL or cryptographic digest may identify an external/input artifact without copying that artifact into a repository. Provenance should preserve a **source locator** sufficient to return to the evidence when lawful and still available: canonical/public URL when available, source or dataset name, public record/document identifier, relevant query or record locator, access method, retrieval timestamp, and content hash when one can be computed. For user-supplied material that has no public source, retain only a non-secret input reference/hash rather than the raw file.

A source locator is a reproducibility trail, not a copy of the evidence and not authority to bypass access controls. If a public URL later disappears, ARCA may preserve the historical locator and hash as provenance while recording that the source is no longer retrievable.

Temporary executor artifacts must follow their existing bounded retention policy and are not an investigative archive.
