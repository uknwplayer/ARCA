...
| Correlação PNCP ↔ execução financeira M2/M5-A/M5-B/M5-C/M5-D/M5-E | sim, M2 sintético + M5 pré-correlação + pós-custódia + observação de schema | sim | parcial: primeiro dado financeiro Portal 2xx custodial obtido | não | run 35917902630; 1 registro; schema observado; `parserAdmitted=false` | admitir parser/normalizador offline e só então alimentar Fase B com evidência live |
| M5-C Observador estrutural do schema Portal | sim | sim | sim: schema financeiro observado no run 35917902630 | não | `SCHEMA_OBSERVED`; `observedSchemaSha256=79f6c837...666f6`; values/raw bytes excluídos | implementar e revisar parser sem novo GET |
