...
| M5-M Cobertura PNCP contratos/empenhos | sim, query runtime hash-bound | sim | run 36036733351: 2 GETs, 400/400; diagnóstico 36037934342 | não | runtime exige `pagina`; correção `pagina=1`; manual 13.10 não lista query; zero novo GET | CI/merge → novo preflight → novo candidato → nova autorização |
