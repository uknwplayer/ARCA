...
| M5-N1 Descoberta PNCP de itens | sim, plano privado hash-bound + cost policy | sim | preflight 060 obsoleto por `tamanhoPagina=10`; novo live ainda não executado | não | GET público PNCP `NO_MONETARY_CHARGE_OBSERVED`; 2 alvos; `pagina=1&tamanhoPagina=10`; zero retries; sem gate humano por request | CI/merge → novo preflight → autoexecução controlada → custódia → truncamento/M5-N2 |
