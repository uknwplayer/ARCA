...
| M5-M Cobertura PNCP contratos/empenhos | sim, plano privado hash-bound | sim, testes sintéticos | não executado live | não | 2 alvos máximos; `maxRequests=2`; `retries=0`; fornecedor + referência PNCP + contrato/empenho esperados | merge → preflight privado → autorização explícita dos 2 GETs |
