...
| Correlação PNCP ↔ execução financeira M2/M5-A/M5-B/M5-C/M5-D/M5-E/M5-F | sim, M2 sintético + M5 pré/pós-custódia + schema + parser sintético | sim | parcial: primeiro dado financeiro Portal 2xx custodial obtido | não | run 35917902630 + parser V1 preso ao schema `79f6c837...666f6` | admitir normalização custodial M5-G e só então alimentar Fase B com evidência live |
...
| M5-G Admissão da normalização custodial | sim, gate candidato | sim | candidato canônico admitido | não | `candidateSha256=0bcf1806...e97f9`; decisão `ADMIT_FOR_CUSTODIAL_NORMALIZATION` | decisão consumida no M5-H; rede/publicação/correlação seguem bloqueadas |
| M5-H Normalização custodial offline | sim, executor local-only | sim, envelope sintético + autorização canônica | não executado nos bytes live | não | candidato `0bcf1806...e97f9` admitido; executor sem download/HTTP; live pendente | co-localizar envelope+passphrase sem rede ou autorizar transporte privado separado |
...
| Portal da Transparência / captura M4b | sim, fixture + transporte + preview + custódia HTTP V0.3 + M5-D + Gate 040 + M5-E + Gate 045/046 | sim | sim, limitada: auth 2xx e primeiro `documentos-relacionados` 2xx custodial | não | run 35917902630; HTTP 200; 1 registro; 440 bytes; `STORED_PRIVATE`; `SCHEMA_OBSERVED` | parser/normalizador ainda não admitido; nenhum novo GET autorizado |
...
| Custódia criptografada | sim | sim | sim | parcial | Portal auth 35910916588 + financeiro 35917902630; receipt financeiro `1ed2cadf...4cbc`; `STORED_PRIVATE` | rotação/gestão operacional e WORM futuro |
