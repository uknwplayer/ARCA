...
test("Portal authorization 003 is restricted to issue #104 and exact creator token",()=>{
  assert.match(workflow,/issue_comment:/);
  assert.match(workflow,/github\.event\.issue\.number == 104/);
  assert.match(workflow,/github\.event\.issue\.state == 'open'/);
  assert.match(workflow,/github\.event\.comment\.user\.login == 'uknwplayer'/);
  assert.match(workflow,/ARCA_PORTAL_LIVE_AUTHORIZATION_003_EXECUTE/);
});

test("Portal authorization 003 pins reviewed revision and scope",()=>{
...
test("Portal authorization 003 does not expose raw document code and uses required secrets",()=>{
...
test("Portal authorization 003 has no schedule, push or pull-request trigger",()=>{
  assert.doesNotMatch(workflow,/^\s+(schedule|push|pull_request|workflow_dispatch):/m);
  assert.match(workflow,/cancel-in-progress:\s*false/);
});


test("Portal authorization 003 binds probe revision to the checked-out commit",()=>{
