...
from .adapter import DispatchRef, ExecutionProviderAdapter, ProviderTransientError
...
class ExecutorMeshDispatcher:
    def __init__(
        ...
        adapters: dict[str, ExecutionProviderAdapter],
    ...
    def dispatch(self, job: JobRequest) -> DispatchDecision:
        ...
        if existing is not None:
            ...
            ranked = next(
                (item for item in self.scheduler.rank(job) if item.descriptor.executor_id == descriptor.executor_id),
                RankedExecutor(descriptor, 0, descriptor.estimated_cost_microunits(job), descriptor.estimated_runtime_seconds(job), ()),
            )
